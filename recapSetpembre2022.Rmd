...
```
  ...
  One needs to notes that in theory (and in practice), Oli's estimations compute the biggest difference within the sample of the group studied. Thus, the red vertical lines should be _always_ at the end of the curve. It is not the case because the kernel estimation "invent" data after the end of the measurment to smooth the curve. In this version we will still use the old version of the threshold, though I have a problem with it it doesn't give me exactly the same result than before.  Maybe because i miss some data?
...
As when creating the consistency matrix, there are plenty of ways to measure the similiarit/distance of composition of each site give the families of artefact it has. For now, and following Radivojevic & Grujic will wi count how many artefact two sites have in common. For exemple, two big sites has HL28 and NGO
```{r countTypesExample,out.width="50%"}
...
for(gp in 1:length(nomin.sitecluster)){
...
plot(mya[,ratioiso[3:2]],bg=sitecol[mya.clus$membership],pch=21,cex=2, main="Full by plot for all Myanmar")
...
```{r,out.width="50%"}
for(gp in 1:length(mya.sitecluster)){
...
plot(isotopes[,ratioiso[3:2]],bg=sitecol[isotopes.clus$membership],pch=21,cex=2,main="Full biploe for whole dataset")
plot(isotopes[,ratioiso[c(3,1)]],bg=sitecol[isotopes.clus$membership],pch=21,cex=2)

ratiolim=list(c(17.7,19.3),c(15.45,15.85),c(37.5,40))
names(ratiolim)=rev(ratioiso)
plot(isotopes[,ratioiso[3:2]],bg=sitecol[isotopes.clus$membership],pch=21,cex=2,xlim=ratiolim[[1]],ylim=ratiolim[[2] ],main="Limited range biploe for whole dataset")
...
for(gp in 1:length(isotopes.sitecluster)){
...
There are multiples axes we can follow, to check, explore and make things better here and there. There are still _some_ problems with the consistency check: using the method of adding link when the three ratio are consistent, we get some "chains" of consistency the can spread very far on the 206.204 axis ; check the grey family on the bottom of the graph in the section 3 for exemple. 

Some techniques likes DBScan may help to cut using points density, but something still needs to be done. Nonetheless I think there is something else to solve at a higher level and that will help us to solve the problem regarding the next step : how, once we get a satisfactory consistency between isotopes composition, do we group/link the sites - geographical area?

And I think that somethings that could help (and that will help also to know when we are roughly satisfy with the consistency grouping) is to  think about some questions the analysis we do will answers. I guesss that the map you showed Oli is a first start but we need to presices things: do we want to find if the network of road correspond to some specific communities (ie specific set of artefacts famillies that 'travel' together?" do we want to be able to define geographical regions with specificity (more diverse than the other regions, connecting different communities, central to some communities,....). 
