...
library(sf)
seashapefile=st_read("Cartes/allseaborder.shp")
...
seashapefile.cr=st_crop(seashapefile,st_buffer(st_as_sf(as.data.frame(coordsites),coords=1:2,crs=st_crs(seashapefile)),dist=100))
for(i in 1:4){
...
seashapefile.cr=st_crop(seashapefile,st_buffer(st_as_sf(as.data.frame(coordsites),coords=1:2,crs=st_crs(seashapefile)),dist=10))
for(i in 1:4){
    sites.graph=allgraphs[[i]]
    V(sites.graph)$label=unique(bigsite$Site.abreviation)
    V(sites.graph)$color=allcommus[,i]
