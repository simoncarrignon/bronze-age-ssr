...
adjmat=matrix(0,nrow=nrow(cleanlist),ncol=nrow(cleanlist))
for(i in 1:(nrow(cleanlist)-1)){
    for(j in (i+1):nrow(cleanlist)){
        adjmat[j,i]=adjmat[i,j]=sum(sapply(ratioiso,function(iso)same(cleanlist[c(i,j),iso],threshold=recomp[iso])))
    }
}
...
limited[limited<2]=0
limited[limited>=2]=1
graph=graph_from_adjacency_matrix(limited,diag=F,weighted=T)
V(graph)$label=cleanlist$Site.abreviation
V(graph)$size=8
