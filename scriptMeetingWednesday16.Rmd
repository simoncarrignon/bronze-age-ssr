...
allgroups=sapply(c(NULL,1:3,function(m)cluster_louvain(graphFromAdj(limited,mask=m,label=cleanlist$Site.abreviation))$membership)
for(i in 1:3){
    graph=graphFromAdj(limited,mask=i,label=cleanlist$Site.abreviation)
...
matdim2=creatSiteAdjMat(pryce,pryce$River.basin,allgroups[,3],match=simpmatch)
sites.graph=graph.adjacency(1/(matdim2^2), mode = "undirected", weighted = TRUE, diag = FALSE)
...
E(sites.graph)$width=E(sites.graph)$weight*100
