...
```{r}
epsilon=0.0001
datappm=read.csv("data/Table S1.csv")
...
coordsites=t(sapply(sites,function(lbl)apply(unique(getSitesCoordiates[getSitesCoordiates$siteperiod == lbl,c("longitude","latitude")]),2,mean)))
...
V(sites.graph)$label.color="black"
sites.communities=cluster_louvain(sites.graph)$memberships[1,]
...
plot(sites.graph,vertex.color=sites.communities)
...
```{r,results="asis"}
orig=read.csv("data/Table S2.csv")
