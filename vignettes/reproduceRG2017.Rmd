---
title: " R-version of Community structure of copper supply networks in the prehistoric Balkans: An independent evaluation of the archaeological record from the 7th to the 4th millennium BC"
...
```{r}
epsilon=0.0001
datappm=read.csv("data/RG_table_s1.csv")
...
getSitesCoordiates=unique(cbind.data.frame(siteperiod,longitude,latitude))

sites.graph=graph.adjacency(matSP, mode = "undirected", weighted = TRUE, diag = FALSE)
V(sites.graph)$label=as.character(sites)
V(sites.graph)$label.cex=.5
V(sites.graph)$size=8
V(sites.graph)$frame.color="black"
V(sites.graph)$frame.width=.5
V(sites.graph)$label.color="black"
louvclus=cluster_louvain(sites.graph)
sites.communities=louvclus$memberships[nrow(louvclus$memberships),]
V(sites.graph)$color=sites.communities
sites.graph=set_graph_attr(sites.graph,"layout",as.matrix(getSitesCoordiates[,2:3]))
E(sites.graph)$color=sites.communities[head_of(sites.graph,E(sites.graph))]
plot(sites.graph)
...
```{r}
par(mar=c(0,0,2,0),oma=c(0,0,0,0),xpd=NA)
for(period in unique(datappm$absolute.period)){
    subperiod=induced_subgraph(sites.graph,V(sites.graph)[grep(period,label)])
    subperiod=induced_subgraph(subperiod,degree(subperiod)>0)
    plot(subperiod,main=period)
    box(which="outer")
}
```


Compare the communities found with original results ; any differences between the group computed here and the results from the papaer are outlined in red.

```{r,results="asis"}
orig=read.csv("data/RG_table_s2.csv")
orig$Modularity.No.=orig$Modularity.No.+1
newds=cbind.data.frame(Label=V(sites.graph)$label,newcommunities=sites.communities)
