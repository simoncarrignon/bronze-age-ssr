...
par(mar=rep(0,4))
library(sf)
seashapefile=st_read("maps/allseaborder_lowq.shp")
plot(seashapefile,lwd=0.1,reset=F)
...
par(mar=c(0,0,0,0))
plot(seashapefile,lwd=0.1,reset=F)
points(datappm$Easting...Longitude,datappm$Northing.Latitude,bg=rainbow(ncommu,alpha=.6)[ppms.louvain$membership],pch=21)
```
