...
myanmar[,17:19]=apply(myanmar[,17:19],1,function(i)i/sum(i))
only=myanmar[,c(17:19)]
...
par(xpd=T)
ggtern(onlys ,aes(A,B,C))+theme_bw()+ geom_point(color=as.numeric(as.factor(myanmar$Region)))+geom_mask()
ggtern(only ,aes(A,B,C))+
geom_density_tern(aes(fill=as.numeric(as.factor(myanmar$Region))),bins=10) + geom_point(color=as.numeric(as.factor(myanmar$Region)))

#legend("topright",legend=unique(as.factor(myanmar$Region)),col=unique(as.numeric(as.factor(myanmar$Region))))

ggtern(onlys ,aes(A,B,C,color=myanmar$Region)) +
  theme_bw() + geom_mask() +
  geom_point()
...
ggtern(only ,aes(A,B,C))+geom_point(color=as.numeric(as.factor(myanmar$Region)))+
...
geom_density_tern(aes(fill=..level..),bins=5) +
 geom_point()
plot(myanmar[,15:16],col=as.factor(myanmar$Region),pch=20)
plot3D(myanmar[,15],myanmar[,16],myanmar[,19])
