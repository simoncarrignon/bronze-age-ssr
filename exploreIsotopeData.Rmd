...
pryce=pryce[!is.na(pryce[,15]),]#useless with the new cleaner dataset
isotopes=pryce[,ratioiso]
pryce.pca=prcomp(pryce[,ratioiso])
...
text(pryce.pca$x[,1],pryce.pca$x[,2],pryce$River.basin,pch=21,bg=rainbow(10,alpha=.6)[pryce.kclust],cex=.4)
...
points(pryce$Easting...Longitude,pryce$Northing.Latitude,pch=21,bg=rainbow(10,alpha=.6)[pryce.kclust])

wholeSEA=st_union(seashapefile)
plot(wholeSEA[,1],reset=F)
points(pryce$Easting...Longitude,pryce$Northing.Latitude,pch=21,bg=rainbow(10,alpha=.6)[pryce.kclust])

text(pryce[,19],pryce[,18],pryce$Site,pch=21,bg=rainbow(10,alpha=.6)[pryce.kclust],cex=.4)

## fin duplicate lines => now obsolete 
#ttt=dist(isotopes)
#ttt=as.matrix(ttt)
...
```{r}
pryce$kclust=pryce.kclust 
qualities=pryce[pryce$Artefact.type %in% c("Slag","Ingots","slag","ingots"),]
...
pryce[,17:19]=apply(pryce[,17:19],1,function(i)i/sum(i))
only=pryce[,c(17:19)]
...
geom_density_tern(aes(fill=..level..),bins=5) +
 geom_point()
plot(pryce[,15:16],col=as.factor(pryce$Region),pch=20)
plot3D(pryce[,15],pryce[,16],pryce[,19])
