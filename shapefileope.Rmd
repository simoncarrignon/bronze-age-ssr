...
allryver=st_read("Cartes/as_bas_30s_beta/as_bas_30s_beta.shp")
allryver=st_make_valid(allryver)
allryver=st_crop(allryver,st_buffer(alldata.sf,dist=1000))
plot(allryver)
allryver=st_read("Cartes/river_basin_20s_cropped.shp")
...
SEA=st_read("Cartes/allseaborder_lowq.shp")
...
st_write(subsetriver,"Cartes/riverbasin_sitelimited.shp")
st_write(allryver,"Cartes/river_basin_lowq.shp")

allryver=rmapshaper::ms <- simplify(allryver,keep=0.000001)

ma=st_read("Cartes/MALAYSIA_adm/MYS_adm0.shp")
lk=st_read("Cartes/SRILANKA_adm/LKA_adm0.shp")
ma=rmapshaper::ms_simplify(st_geometry(ma))
lk=rmapshaper::ms_simplify(st_geometry(lk))

SEA=st_union(st_union(ma,SEA),lk)
st_write(SEA,"Cartes/allseaborder_lowq.shp")
